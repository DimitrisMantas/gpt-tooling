...
## Automatic base routing for an annual report
...
- Writer applies the base branch.
...
- Writer loads the technical extension.
- The artifact follows applicable venue and disciplinary conventions.
- Writer does not add analyses or results absent from the supplied record.
...
- Writer uses the technical branch.
- The report preserves requirements, methods, evidence, uncertainty, and accepted limitations.
- Writer does not request generic extra metrics without a technical obligation from Engineer.
...
- Writer applies both the base and technical policies.
...
- Writer identifies that the claim lacks available uncertainty information when this affects interpretation.
- Writer does not calculate or invent confidence intervals.
- The gap is returned to Engineer or the user as a technical-content issue.
...
- Writer does not manufacture significance, novelty, or a directional preference.
...
- Writer does not collapse the evidence into a uniformly positive narrative.
...
- Writer uses established practice for the relevant discipline.
...
- Writer improves the words.
- The project formatter governs physical wrapping.
- Writer does not enforce an absolute no-wrap rule inside the source file.
...
- Writer follows the target style and may use `20–40 m`.
...
- Writer treats sentence length as a diagnostic threshold.
...
- Writer uses concise natural technical prose.
...
- Writer does not treat Humanizer and its Wikipedia basis as independent evidence.

## Technical feedback boundary

Prompt:

> A reviewer asks Writer to add a new experiment so the report has a stronger conclusion.

Expected behavior:

- Writer treats the request as a technical proposal for Engineer to evaluate.
