...
function selfTest() {
  ...
  const required = [
    ...
    /Quire governs every natural-language surface\./
  ];

  for (const rule of required) {
    if (!rule.test(skill)) throw new Error(`The required Plinth rule is missing: ${rule}.`);
  }
  if (!instructions().includes(skillPath)) throw new Error("The hook omits the Plinth skill route.");
