...
11. Derive metrics, diagnostics, experiments, and reviews from evidence obligations. Start a comparison plan with the decision to be made and the evidence that could change it. Distinguish selection evidence from descriptive characterization, debugging diagnostics, and exploratory analysis; give each retained item a decision-relevant purpose rather than a default place in a checklist. Add one only when it resolves a material question, protects against a concrete blind spot, can change an engineering action, or supports a necessary claim.
