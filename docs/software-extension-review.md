...
The three added behavioral cases exercise cross-module contract preservation, non-Python repository compatibility, and typing/runtime/prose boundaries. Run them with `node scripts/eval.js run-local <case-id>`. The snapshot records policy hashes and supplies complete policy text through stdin to a clean candidate context, with a separate clean grader. The initial path-based experiment encountered blocked Windows sandbox reads, so its apparent semantic passes do not establish that the policies loaded. Inline delivery addresses that test limitation; it does not test progressive discovery or hook activation. These are representative policy regressions, not a measurement of every model or every generated program. Run the installed suite after refreshing the local plugin installation to verify lifecycle discovery with the revised policies.
