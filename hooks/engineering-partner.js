...
const skillPath = path.join(root, "skills", "engineering-partner", "SKILL.md");
...
function instructions() {
  return [
    "ENGINEERING PARTNER DISCIPLINE ACTIVE",
    "",
    `Apply Engineering Partner to every engineering task. If the complete skill at "${skillPath}" has not been loaded in this agent context, read it before acting. Then load only the references relevant to the task.`,
    ...
    "Ponytail governs minimum code and dependency scope. Engineering Partner governs engineering semantics, evidence, conventionality, quality, risk, and defensibility. Writing governs natural language."
  ].join("\n");
}
...
function selfTest() {
  ...
  const required = [
    ...
    /Ponytail governs code and coding decisions\./,
    /Writing governs every natural-language surface\./
  ];

  for (const rule of required) {
    if (!rule.test(skill)) throw new Error(`The required Engineering Partner rule is missing: ${rule}.`);
  }
  if (!instructions().includes(skillPath)) throw new Error("The hook omits the Engineering Partner skill route.");
  ...
  process.stdout.write("Engineering Partner hook checks passed.\n");
}
...
if (command === "test") {
  ...
  process.stderr.write("Usage: engineering-partner hook test | inject <event>\n");
  process.exitCode = 2;
}
